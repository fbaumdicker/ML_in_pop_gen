""" script to create violin plots for simulated data from recombination map
for human chromosome 2. adaptive neural network, linear neural network,
watterson's estimator and the iterative version of Futschik's and Gach's
estimator are considered.

Note: SFS needs to be computed via the script sim_chr2.py before running
this script.
"""
...
# specify ranges for recombination rates for which violin plots should be
# created. in this case 3 classes: 0<rho<=1, 30<rho<=40 and rho>150.
...
input_data = numpy.load(
    "../data/simulations/SFS_along_chr2_seed_" + str(seed) + ".npz",
    allow_pickle=True,
)
...
# sort SFS into 3 classes by mean recombination rates of disjoint
# 70kb windows of chr2.
...
for i in range(0, math.floor(len(df1) / 10)):
    ...
    sum_rho = (
        numpy.sum(
            rate_map.get_rate(range(int(mut_bin[i * 10]), int(mut_bin[(i * 10) + 10])))
        )
...
for i in range(0, num_classes):
    ...
    ann_pred = keras.models.load_model(
        "../data/saved_NN/adaptive_NN_1hl_200_n_"
        + str(num_sample)
        + "_rep_600000_rho_var_theta_random-100_1",
        compile=False,
    ).predict(X_test)
    ann_pred_classes.append(numpy.array([ann_pred[j, 0] for j in range(0, len(df2))]))
    lin_nn_pred = keras.models.load_model(
        "../data/saved_NN/Linear_NN_n_40_rep_600000_rho_var_theta_random-100",
        compile=False,
    ).predict(X_test)
    lin_nn_pred_classes.append(
        numpy.array([lin_nn_pred[j, 0] for j in range(0, len(df2))])
    )
...
violin_plot1 = sns.violinplot(
    data=data1, palette=my_color, inner=None, saturation=0.6, cut=0
)
sns.boxplot(
    data=data1,
    palette=my_color,
    width=0.035,
    boxprops={"facecolor": my_grey, "color": my_grey, "zorder": 2},
    ax=violin_plot1,
    flierprops=flierprops,
    whiskerprops=dict(color=my_grey, linewidth=1.5),
...
violin_plot2 = sns.violinplot(
    data=data2, palette=my_color, saturation=0.6, inner=None, cut=0
)
sns.boxplot(
    data=data2,
    palette=my_color,
...
violin_plot3 = sns.violinplot(
    data=data3, palette=my_color, saturation=0.6, inner=None, cut=0
)
sns.boxplot(
    data=data3,
    palette=my_color,
